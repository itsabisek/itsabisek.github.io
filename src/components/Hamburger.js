...
const Hamburger = () => {
    ...
    return (
        <div className={`lg:hidden fixed flex flex-col z-50 top-0 right-0 text-whiteBg ${isHamOpen ? 'bg-primaryBg duration-500' : 'bg-greyBg duration-500'}`} onClick={() => setisHamOpen((isHamOpen) => !isHamOpen)}>
...
const ExpandList = ({ isHamOpen }) => {
    ...
    return (
    <div className={`flex flex-col justify-start items-start h-screen ${isHamOpen ? 'animate-in slide-in-from-right duration-500 fade-in-0 opacity-100 w-screen' : 'animate-out slide-out-to-right duration-500 fade-out-100 opacity-0 w-0'} `}>
        {links.map(({ id, link }) => (
            <a href={`#${link}`} className={`flex relative p-8 m-4 justify-center items-center ${isHamOpen ? '' : 'hidden'}`}>
                <p>{link}</p>
                <div className="absolute w-1 bottom-0 left-0 right-0 bg-neonBg"></div>
            </a>
        ))}
